        ...
        if (type === "start") {
            ...
            return {
                ...
                disabledMinutes: (selectedHour: number) => {
                    if (
                        checkSelectedHour(doctorSchedule, selectedHour)
                            ?.index &&
                        moment(
                            checkSelectedHour(doctorSchedule, selectedHour)
                                ?.hour,
            ...
            let removeHours = range(0, 24).filter((num) => {
                if (
                    ...
                    let nearestStartTime = doctorSchedule.find(
                        ({ start_time }: any) => {
                            return (
                                moment(selectedStartTime, "HH:mm").hour() <
                                moment(start_time, "HH:mm").hour()
            ...
            return {
                ...
                disabledMinutes: (selectedHour: number) => {
                    if (selectedHour < 0) {
                        ...
                        checkSelectedHour(mergedSelectedToArray, selectedHour)
                            ?.index &&
                        moment(
                            checkSelectedHour(
                                mergedSelectedToArray,
                                selectedHour
                            )?.hour,
                            "HH:mm"
                        )
        ...
        <Modal show={show} onClose={onClose} {...rest}>
            <div className="space-y-8">
                ...
                <Form
                    ...
                    onFinishFailed={(data) => {
                        scroller.scrollTo(
                            data?.errorFields[0]?.name[0].toString(),
                            {
                                smooth: true,
                                offset: -50,
                                containerId: rest?.id,
