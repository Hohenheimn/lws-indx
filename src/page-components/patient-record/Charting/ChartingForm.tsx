...
export default function ChartingForm({
  ...
  const GetAnnotationWithValues = (
    annotate: AnnotationType,
    annotateArray: AnnotationType[]
  ) => {
    let annotateWithValue: AnnotationType = {
      tooth_no: 0,
      tooth_position: "",
      annotations: [],
    };
    annotateArray.map((item: AnnotationType) => {
      if (annotate.tooth_no === item.tooth_no) {
        annotateWithValue = annotate;
        return;
      }
    });
  ...
  const setAnnotationValues = () => {
    ...
    defaultAnnotation.map((rowPosition: AnnotationType) => {
      if (rowPosition.tooth_position === "upper_left") {
        const getFromFunc = GetAnnotationWithValues(
          rowPosition,
          Teeth(age, form.getFieldValue("chart_view")).UpperLeft
        );
      ...
      if (rowPosition.tooth_position === "upper_right") {
        const getFromFunc = GetAnnotationWithValues(
          rowPosition,
          Teeth(age, form.getFieldValue("chart_view")).UpperRight
        );
      ...
      if (rowPosition.tooth_position === "lower_left") {
        const getFromFunc = GetAnnotationWithValues(
          rowPosition,
          Teeth(age, form.getFieldValue("chart_view")).LowerLeft
        );
      ...
      if (rowPosition.tooth_position === "lower_right") {
        const getFromFunc = GetAnnotationWithValues(
          rowPosition,
          Teeth(age, form.getFieldValue("chart_view")).LowerRight
        );
  ...
  const ProcedureSetDefaultValue = (
    ...
    setArrayValues([...Filter, ...valuesToSet]);
  };
      ...
      <div className="space-y-8">
        ...
        <Form
          ...
          onFinish={(values) => {
            ...
            values.legend_occlusions = JSON.stringify(values.legend_occlusions);
            values.legend_appliances = JSON.stringify(values.legend_appliances);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            >
              ...
              >
                ...
                <Select.Option value={"Periodontal"}>Periodontal</Select.Option>
              </Select>
            </Form.Item>
            <Form.Item
              label="Branch"
